...
router.post('/', (req, res, next) => {
  ...
  let { username, password, nikename, mobile, email} = req.body;

  //1.5 设定必传参数
  // if (!username && !password ) {
  if ((!username && !password) || (!mobile && !password) ) {
    res.send({
      err: 1,
      msg: '用户名，密码为必传参数'
    });
        ...
        if (err) {
          ...
          if (result.length === 0) {//不存在
            ...
            collection.insertOne({
              nikename, username, fans, follow, time, icon, password: hash, mobile, email
            }, (err, result) => {
              if (!err) {
